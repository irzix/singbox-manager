...
export async function generateRealityKeyPair(): Promise<{
  ...
  try {
    ...
    if (!output) throw new Error('sing-box not found');
  ...
  console.warn(
    '⚠️  Generated fallback keys. For production, install sing-box and regenerate.'
  );
 ...
 */
export function generateShortIds(count: number = 4): string[] {
