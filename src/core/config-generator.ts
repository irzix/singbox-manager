...
import { generateUUID, generateShortIds, generateRealityKeyPair } from '../utils/crypto.js';
 ...
 */
export async function generateRealityConfig(
  dest?: string
): Promise<RealityConfig> {
 ...
 */
export async function generateDefaultServerConfig(
  host: string,
  port: number = 443
 ...
 */
export function toSingboxConfig(
  serverConfig: ServerConfig,
  users: User[]
): SingboxConfig {
  const inbounds: SingboxInbound[] = serverConfig.inbounds.map((inbound) => {
    const base: SingboxInbound = {
 ...
 */
export async function loadConfig(path: string): Promise<SingboxConfig> {
  const content = await readFile(path, 'utf-8');
  return JSON.parse(content) as SingboxConfig;
}
